...
- two small public RNA-seq samples,
- lightweight command-line tools.

You will run:
1. VM setup on Compute Engine,
2. tool installation,
3. read quality checks,
4. transcript quantification with Salmon,
5. transcript-to-gene mapping extraction,
6. review and optional upload of results to Cloud Storage.
...
## Datasets and reference used in this tutorial

This tutorial uses:
- **Public RNA-seq sample accessions:** `SRR453566` and `SRR453567` (single-end; *Saccharomyces cerevisiae*),
...
- **Transcript-to-gene mapping output:** `results/tx2gene.tsv`.

Why this choice:
- yeast reference is small,
- two samples enable simple side-by-side comparison,
- still suitable for a VM with **<= 8 GB RAM**.
...
- data download + QC + quantification for two samples: ~15-35 min,
- tx2gene mapping extraction: <1 min.
